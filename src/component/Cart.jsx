...
export const Cart = ({allPizzas, setAllPizzas, countPizzas, setCountPizzas, total, setTotal}) => {
  ...
  <div className="cart_section">
    <h1 className="cart_title mb-4">Carro de Compra</h1>
    <div className="cart_container">
      {allPizzas.map((pizza) => (
        ...
        <div className="cart_container-total">
          ...
          {allPizzas.length === 0 ? 
          <p className = "cart_vacio-text">Carrito de compras vacio</p> : 
          <button type="button" className="btn_total btn btn-info">Pagar</button> }
        </div>
    </div>
  </div>
  )
